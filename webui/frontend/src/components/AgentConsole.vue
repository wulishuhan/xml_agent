...
function getLabel(item) {
    ...
    if (type === "stdout") return getActionTag(item) === "answer" ? "Answer" : "Agent";
    return "Event";
}
...
/** * 从 agent 事件里取出 XML Action 名（read / write / exec / answer / done）。 * session 记录里 record.event 保存了原始 agent 事件。 */ function getActionTag(
    item
) {
...
/** * 是否可折叠：内容较长、行数较多，或本身就是结构化 JSON。 */ function isCollapsible(item) {
    const text = contentText(item);
    if (!text) {
        return false;
    }
...
/** * 折叠时展示的摘要：取前几行 / 前若干字符，最多 3 行。 */ function getPreview(item) {
    const text = contentText(item);
    if (!text) {
        return "";
    }
