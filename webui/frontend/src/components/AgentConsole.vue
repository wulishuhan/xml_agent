<template>
    <section class="agent-stream">
        <div class="stream-header">
            ...
            <button v-if="output.length" class="stream-clear" type="button" @click="$emit('clear')">
                Clear
            </button>
        </div>
...
function getLabel(type) {
    const labels = { system: "System", stdout: "Agent", stderr: "Error", error: "Error" };
    return labels[type] || "Event";
}
function getIcon(type) {
    const icons = { system: "◆", stdout: "›", stderr: "!", error: "×" };
    return icons[type] || "·";
}
